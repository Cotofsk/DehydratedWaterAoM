...
const BuildOrderView: React.FC = () => {
  ...
  return (
    <div className="bg-wood-dark/80 p-6 rounded-lg border-2 border-sandy-gold/30">
      <div className="flex justify-between items-start mb-6">
        <Button 
          onClick={goBack}
          variant="outline"
          className="border-sandy-gold/30 hover:bg-sandy-gold/20 hover:border-sandy-gold text-sandy-gold"
        >
          <ArrowLeft className="h-4 w-4 mr-2" />
          Back
        </Button>
        ...
        <div className="flex items-center gap-4">
          <div className="flex flex-col bg-wood-dark/90 rounded-lg p-3 shadow-md border border-sandy-gold/30">
      ...
      <div className="mb-6">
        <h1 className="text-4xl font-cinzel text-sandy-gold font-bold mb-2">
          {buildOrder.name}
        </h1>
        ...
        <div className="overflow-hidden rounded-lg border border-sandy-gold/30">
          <table className="min-w-full divide-y divide-sandy-gold/30">
            <thead className="bg-wood-dark/90">
              <tr>
                <th className="px-4 py-3 text-left text-xs font-medium text-sandy-gold uppercase tracking-wider">
                  #
                </th>
                <th className="px-4 py-3 text-left text-xs font-medium text-sandy-gold uppercase tracking-wider">
                  Main Action
              ...
              {entries.map((entry) => (
                <React.Fragment key={entry.id}>
                  <tr className="hover:bg-wood-dark/95">
                    ...
                    <td className="px-4 py-3 whitespace-nowrap text-sm text-sandy-gold font-semibold">
                      {entry.mainAction}
                    </td>
                  ...
                  {entry.miscellaneousAction && (
                    <tr className="bg-wood-dark/70">
                      ...
                      <td colSpan={2} className="px-4 py-2 text-sm text-sandy-gold/70 italic">
                        {entry.miscellaneousAction}
                      </td>
                    </tr>
                  )}
